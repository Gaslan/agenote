...
export default function Folders({folders, onFolderDelete}: FoldersProps) {
  ...
  function handleDeleteButtonClick() {
    onFolderDelete(anchorEl?.getAttribute('data-folder-id') ?? '')
    handleClose()
  }
    ...
    <>
      <div>
        {folders && folders.map(folder => (
          <Box onClick={() => {handleFolderClick(folder)}} className={selectedFolder ? ((selectedFolder as Folder).id == folder.id ? 'active' : '') : ''} key={folder.id} sx={{p: '4px 4px', cursor: 'pointer', '&:hover': {backgroundColor: 'rgba(0,0,0,.05)'}, '&:hover .MuiButtonBase-root': {visibility: 'visible'}, '&.active': {backgroundColor: 'rgba(0,0,0,.08)'}}}>
            <Stack direction={'row'} alignItems={'center'}>
              <Box width={'24px'} height={'24px'} borderRadius={'50%'} bgcolor={folder.cover} mr={'8px'} ml={'4px'}>

              </Box>
              <Typography variant="body2" component={'span'} flexGrow={1}>
                {folder.name}
              </Typography>
              <IconButton onClick={handleClick} data-folder-id={folder.id} disableRipple disableTouchRipple sx={{visibility: 'hidden', '&:hover': {color: '#111'}}}>
